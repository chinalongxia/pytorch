...
class Add(InplaceFunction):
    ...
    def backward(ctx, grad_output):
        return grad_output, maybe_view(grad_output, ctx.b_size)
...
class Sub(InplaceFunction):
    ...
    @staticmethod
    def backward(ctx, grad_output):
        return grad_output, maybe_view(grad_output.neg(), ctx.b_size)
...
class AddConstant(InplaceFunction):
    ...
    @staticmethod
    def forward(ctx, a, b, inplace=False):
        tensor, constant, _ = sort_args(a, b)
        if inplace:
...
class MulConstant(InplaceFunction):
    ...
    @staticmethod
    def forward(ctx, a, b, inplace=False):
        tensor, ctx.constant, _ = sort_args(a, b)
        if inplace:
            ctx.mark_dirty(tensor)
    ...
    @staticmethod
    def backward(ctx, grad_output):
        return grad_output.mul(ctx.constant)
...
class DivConstant(InplaceFunction):
    ...
    def backward(ctx, grad_output):
        if ctx.tensor_first:
            return grad_output.div(ctx.constant)
        else:
            v, = ctx.saved_variables
            if ctx.inplace:
                return grad_output.mul(v).mul(v).div_(-ctx.constant)
            else:
                v_rep = v.reciprocal()
                return grad_output.mul_(-ctx.constant).mul(v_rep).mul(v_rep)
...
class PowConstant(Function):
    ...
    def backward(ctx, grad_output):
        if ctx.tensor_first:
            ...
            return grad_output.mul(ctx.constant).mul(var.pow(ctx.constant - 1))
        else:
            var_result, = ctx.saved_variables
            return grad_output.mul(var_result).mul_(math.log(ctx.constant))
