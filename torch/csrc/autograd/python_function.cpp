...
auto PyFunction::apply(const variable_list& inputs) -> variable_list {
  ...
  // strip unnecessary Nones at the end
  auto num_outputs = PyTuple_GET_SIZE(r.get());
  int num_next_functions = next_functions.size();
  if (num_outputs > num_next_functions) {
    bool all_none = true;
    for (int i = num_outputs; i < num_next_functions; i++) {
      all_none &= PyTuple_GET_ITEM(r.get(), i) == Py_None;
    }
    if (all_none) {
      num_outputs = num_next_functions;
      r = PyTuple_GetSlice(r.get(), 0, num_outputs);
      if (!r) throw python_error();
    }
  }
...
struct UnpackedInput {
  UnpackedInput(PyObject* raw_input, THPObjectPtr&& tensor_input, variable_list&& input_vars)
    : raw_input(raw_input)
    , tensor_input(std::move(tensor_input))
...
struct InputFlags {
  InputFlags(FunctionFlags&& flags, THPObjectPtr&& needs_input_grad)
    : flags(std::move(flags))
    , needs_input_grad(std::move(needs_input_grad)) {}
...
std::pair<UnpackedInput, InputFlags> unpack_input(PyObject *args) {
  ...
  THPObjectPtr tensor_input = PyTuple_New(num_args);
  THPObjectPtr needs_input_grad = PyTuple_New(num_args);
  for (int i = 0; i < num_args; i++) {
    ...
    if (!is_variable) {
      ...
      new_arg = arg;
      Py_INCREF(Py_False);
      PyTuple_SET_ITEM(needs_input_grad.get(), i, Py_False);
    } else {
      THPVariable* variable = (THPVariable*)arg;
      new_arg = THPVariable_get_data(variable);
      input_vars.push_back(variable->cdata);
      Py_INCREF(Py_True);
      PyTuple_SET_ITEM(needs_input_grad.get(), i, Py_True);
    }
    PyTuple_SET_ITEM(tensor_input.get(), i, new_arg);
  }

  auto fn_flags = Function::flags(input_vars);
  auto unpacked = UnpackedInput(args, std::move(tensor_input), std::move(input_vars));
  auto flags = InputFlags(std::move(fn_flags), std::move(needs_input_grad));
